...
// Add render tracking at the top of component
let renderCount = 0;
...
const DraggablePokemonMilestoneCard: React.FC<DraggablePokemonMilestoneCardProps> = memo(({ 
  ...
  renderCount++;
  console.log(`🔍 [CARD_RENDER_TRACKER] ${pokemon.name} (ID: ${pokemon.id}): Render #${renderCount} - Index: ${index}, Pending: ${isPending}, Context: ${context}`);
  ...
  // Memoize sortable configuration to prevent recreation
  ...
  if (propsChanged) {
    console.log(`🔍 [CARD_MEMO_TRACKER] ${nextProps.pokemon.name}: Props changed, allowing re-render`);
  }
