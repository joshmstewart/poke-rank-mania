...
export const RankingsSection: React.FC<RankingsSectionProps> = React.memo(({
  ...
  const DroppableRankingSlot: React.FC<{ index: number; pokemon?: Pokemon | RankedPokemon }> = ({ index, pokemon }) => {
    ...
    }, [isOver, index, droppableId, pokemon]);
    ...
    return (
      <div 
        ...
        `}
        onMouseEnter={() => {
          console.log(`🖱️ [MOUSE_DEBUG] Mouse entered slot ${index} (${droppableId})`);
        }}
        onMouseLeave={() => {
          console.log(`🖱️ [MOUSE_DEBUG] Mouse left slot ${index} (${droppableId})`);
        }}
      >
        {pokemon ? (
