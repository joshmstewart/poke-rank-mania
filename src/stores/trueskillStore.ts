...
// The core logic of the store, extracted into a function.
// This allows us to conditionally apply middleware like `persist`.
const storeInitializer = (set, get): TrueSkillStore => ({
  ratings: {},
  pendingBattles: [],
  refinementQueue: [],
  ...
  setSessionId: (newSessionId: string) => {
    ...
    if (oldSessionId !== newSessionId) {
      ...
      set({ sessionId: newSessionId, ratings: {}, totalBattles: 0, sessionReconciled: false }); // Reset local state when changing session
    }
  },
...
// Helper to check for auth token at module load time. This allows us to decide
// whether to enable localStorage persistence *before* Zustand is initialized.
const getSupabaseAuthToken = () => {
  try {
    // Guard against SSR or environments where localStorage is not available.
    if (typeof window === 'undefined' || typeof localStorage === 'undefined') return null;

    const key = Object.keys(localStorage).find(key => key.startsWith('sb-') && key.endsWith('-auth-token'));
    return key ? localStorage.getItem(key) : null;
  } catch (error) {
    console.warn('[trueskillStore] Could not access localStorage to check for auth token.');
    return null;
  }
};

const isPotentiallyAuthenticated = !!getSupabaseAuthToken();

console.log(`[trueskillStore] Initializing store. User is potentially authenticated: ${isPotentiallyAuthenticated}. Persistence is ${isPotentiallyAuthenticated ? 'DISABLED' : 'ENABLED'}.`);

// Conditionally apply the persist middleware.
// - If a user is logged in (or has a token), we DISABLE persistence to prevent
//   local data from overriding fresh cloud data on page load.
// - If the user is anonymous, we ENABLE persistence for offline support.
const storeCreator = isPotentiallyAuthenticated
  ? storeInitializer
  : persist(storeInitializer, {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (state) {
          ...
          // Common hydration logic for anonymous users
          ...
          console.log(`🚨🚨🚨 [SYNC_AUDIT] Zustand hydration complete for anonymous user. Session reconciled: ${state.sessionReconciled}`);
        }
      }
    });

export const useTrueSkillStore = create<TrueSkillStore>()(storeCreator);
