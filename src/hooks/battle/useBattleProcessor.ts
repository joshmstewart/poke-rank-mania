...
export const useBattleProcessor = (
  ...
  const processBattleResult = (selections: number[], battleType: BattleType, currentBattle: Pokemon[]) => {
    ...
    // Make sure the battle is different from the last completed one
    if (lastBattleCountRef.current === battlesCompleted) {
      console.log(`Processing new battle result, incrementing from ${battlesCompleted}`);
      ...
      lastBattleCountRef.current = newBattlesCompleted; // Update our reference
      ...
      if (milestones.includes(newBattlesCompleted)) {
        ...
        setTimeout(() => {
          ...
          // Reset processing state after the new battle has started with additional delay
          setTimeout(() => {
            setIsProcessingResult(false);
          }, 800);
        }, 500);
      ...
      console.log("This battle was already processed, ignoring duplicate");
      setIsProcessingResult(false);
    }
  };
