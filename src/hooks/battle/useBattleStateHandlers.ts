...
export const useBattleStateHandlers = (
  ...
  const handleManualReorder = useCallback((draggedPokemonId: number, sourceIndex: number, destinationIndex: number) => {
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] ===== DRAG AND DROP OPERATION =====`);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Pokemon ID: ${draggedPokemonId}`);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Source position: ${sourceIndex + 1}`);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Destination position: ${destinationIndex + 1}`);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Refinement queue instance:`, refinementQueue);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Current queue before drag: ${refinementQueue.refinementBattleCount} battles`);
    
    // Find the dragged Pokemon to log its name
    const draggedPokemon = finalRankings.find(p => p.id === draggedPokemonId);
    if (draggedPokemon) {
      console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Pokemon name: ${draggedPokemon.name}`);
    } else {
      console.error(`🚨 [MANUAL_REORDER_CONTEXT_DEBUG] Could not find dragged Pokemon ${draggedPokemonId} in finalRankings!`);
      console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] finalRankings length: ${finalRankings.length}`);
      console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] First 5 Pokemon IDs in rankings:`, finalRankings.slice(0, 5).map(p => p.id));
    }
    ...
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Total rankings available: ${rankings.length}`);
    ...
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Checking positions for neighbors: ${positions.join(', ')}`);
    
    positions.forEach(pos => {
      if (pos >= 0 && pos < rankings.length && rankings[pos] && rankings[pos].id !== draggedPokemonId) {
        neighbors.push(rankings[pos].id);
        console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Added neighbor at position ${pos + 1}: ${rankings[pos].name} (${rankings[pos].id})`);
      } else {
        console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Skipped position ${pos + 1}: ${pos < 0 ? 'negative' : pos >= rankings.length ? 'out of bounds' : rankings[pos]?.id === draggedPokemonId ? 'same as dragged' : 'invalid'}`);
      }
    });
    
    // Ensure we have at least some validation battles
    if (neighbors.length === 0 && rankings.length > 1) {
      console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] No neighbors found, adding fallback neighbors...`);
      ...
      for (let i = Math.max(0, destinationIndex - 3); i <= Math.min(rankings.length - 1, destinationIndex + 3); i++) {
        if (rankings[i] && rankings[i].id !== draggedPokemonId && neighbors.length < 3) {
          ...
          console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Added fallback neighbor: ${rankings[i].name} (${rankings[i].id})`);
        }
      }
    }
    
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Final neighbors list: ${neighbors.join(', ')}`);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Total neighbors: ${neighbors.length}`);
    
    // Queue refinement battles - this is where the drag action creates future battles
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Queueing validation battles...`);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Refinement queue before queueing: ${refinementQueue.refinementBattleCount} battles`);
    
    refinementQueue.queueBattlesForReorder(draggedPokemonId, neighbors, destinationIndex + 1);
    
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] ✅ Successfully queued ${neighbors.length} validation battles`);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Total refinement battles in queue AFTER queueing: ${refinementQueue.refinementBattleCount}`);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] Current refinement queue state:`, refinementQueue.refinementQueue);
    console.log(`🔄 [MANUAL_REORDER_CONTEXT_DEBUG] ===== END DRAG AND DROP OPERATION =====`);
